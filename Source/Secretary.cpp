...
Secretary :: Secretary(const Secretary &S){
    ...
    std::vector<Student*> :: const_iterator itr1 = S.Students.begin(); // Const iterator because secretary S is a const variable
...
std::vector<Student*> Secretary :: Degree(void){
    ...
    std::vector<Student*> DegreeReady; // the vector that will contain the students ready to get a degree
    std::vector<Student*> :: iterator it = Students.begin();
    while (it != Students.end()){
...
void Secretary :: operator=(const Secretary &S){
    ...
    std::vector<Course*> :: const_iterator itr3 = S.Courses.begin(); // Const iterator because secretary S is a const variable
    while(itr3 != S.Courses.end()){
...
void Secretary :: StartSemester(){
    ...
    while(it != Students.end()){
        std::cout << "Which subject do you want to add to " << (*it)->GetName() << "(type end if you don't want to add anymore)-> ";
    ...
    while(it2 != Courses.end()){
        std::cout << "Do you want to change the professor of the course " << ((*it2)->GetName()) << "(True/False)-> ";
        std::getline(std::cin>> std::ws , sub);
...
void Secretary :: EndSemester(){
    ...
    while(it != Students.end()){
        ...
        it2 = C.begin();
        while(it2 != C.end()){
            std::cout << "What Grade did " << (*it)->GetName() << " get in " << (*it2)->GetName() << "-> ";
            std::cin >> Grade;
...
void Secretary :: printfiles(void){
    ...
    while(it2 != Students.end()){
        ...
        std::vector<std::string> temp1= (*it2)->getnamemap();// this function returns the courses that the students are taking
        std::vector<float> temp2= (*it2)->getgrademap();// this function returns the grade of the course
