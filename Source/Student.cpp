...
Student :: Student(std::string Name , int age , std::string email, std::string Sex,std::string AM,int semester):Person(Name,Sex,age,email){
...
void Student :: SetAM(std::string AM){
    this->AM = AM;
}
...
std::string Student :: GetAM(void) const{
    return AM;
}
...
std::vector<Course*> Student :: GetSubjects(void) const{
    return Courses;
}
...
void Student :: PrintYear(int Year){
    ...
    if(Year == 0){
        while(it != PassedCourses.end()){
            std::cout << "Course:  "<< it->first << "    Grade: " << it->second.GetGrade() << std::endl;
            it++;
            flag=true;
        }      
    }
    ...
    if(!flag){
        std::cout<< "The student doesn't have any courses in this year"<< std::endl;
    }
    std::cout << std::endl;
}
...
void Student :: PrintSemester(int Semester){
    ...
    if(!flag){
        std::cout<< "The student doesn't have any courses in this semester"<< std::endl;
    }
    std::cout << std::endl;
}
...
void Student :: operator=(const Student &S){
    ...
    PassedCourses.clear();
    std::unordered_multimap<std::string,Grades> :: const_iterator itr = S.PassedCourses.begin();
    while(itr != S.PassedCourses.end()){
        PassedCourses.insert(std::pair<std::string,Grades>(itr->first,itr->second));
        itr++;
    }
...
std::istream &operator>> (std::istream &str , Student *S){
    ...
    std::getline(str >> std::ws , S->Name);//reads a string with the spaces because the name contains and the surname
