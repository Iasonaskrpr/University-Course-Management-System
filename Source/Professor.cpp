...
Professor ::Professor(std::string Name, std::string email, int age,
                      std::string Sex)
    : Person(Name, Sex, age, email) {}

Professor ::Professor(std::string Name, std::string email, int age,
                      std::string Sex, std::vector<Course *> c)
    : Person(Name, Sex, age, email) {
...
std::vector<Course *> Professor ::GetCourses(void) const { return Courses; }
...
std::istream &operator>>(std::istream &str, Professor *P) {
  ...
  std::getline(str >> std::ws,
               P->Name); // reads a string with the spaces because the name
